...
const ModalProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const showModal = React.useCallback(
    (config: ModalConfig) => {
      if (config.buttons) {
        config.buttons.forEach((button, idx) => button.key && ({ key: `${button.key}-${idx}` }));
      }
      setModal(config);
    },
    []
  ...
  return (
    <ModalContext.Provider value={{ showModal, setModal, closeModal, modal }}>
      {children}
      {modal && <ModalDialog {...{ key: modal.id }} config={modal} onClose={() => closeModal(modal.id)} />}
...
const ModalProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  return (
    <ModalContext.Provider value={{ showModal, setModal, closeModal, modal }}>
      {children}
      {modal && <ModalDialog key={modal.id} config={modal} onClose={() => closeModal(modal.id)} />}{/* key is used to unmount the entire modal component and re-mount when the modal changes. This is important for modals */}
    </ModalContext.Provider>
  );
};
        ...
        {config.buttons && config.buttons.length > 0 && (
          <div className="flex flex-col sm:flex-row gap-2 p-6 pt-0 sm:items-center justify-end">
            {config.buttons?.map((button) => (
              <ActionButton key={button.key} {...button} loading={false} />
            ))}
          </div>
        )}
      </div>
...
// Utility: EscapedKeyHandler
const EscapedKeyHandler = ({ onClose }: { onClose?: () => void }) => {
...
// Utility: ToastContainer (wrapper over toast context)
